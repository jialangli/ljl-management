    ...
    <el-card>
      <template #header>
        <div class="card-header">
          <span>培训管理</span>
          <el-button type="primary" @click="handleApply">申请培训</el-button>
        </div>
      </template>

      <el-tabs v-model="activeTab">
        <el-tab-pane label="我的培训" name="my">
          <el-table :data="myTrainings" style="width: 100%">
        ...
        <el-tab-pane label="培训课程" name="all">
          <el-table :data="allTrainings" style="width: 100%">
            ...
            <el-table-column label="操作" width="100">
              <template #default="{ row }">
                <el-button type="text" @click="handleApplyCourse(row)">
                  申请参加
                </el-button>
              </template>
            </el-table-column>
      ...
      <el-dialog
        ...
        :title="dialogType === 'add' ? '申请培训' : '培训详情'"
        width="500px"
      >
        <el-form
          ref="formRef"
...
const handleApplyCourse = (row: any) => {
  // 调用申请API
...
const handleSubmit = async () => {
  ...
  await formRef.value.validate((valid: boolean) => {
    if (valid) {
      // 调用API
      myTrainings.value.push({
        id: myTrainings.value.length + 1,
        ...form,
        status: '待审核'
      })
